...
| FR-MEDIA-001 | 단일 선택 URI의 MIME, dimensions, FPS, profile, codec binding을 fail-closed 검증해야 한다. 4K60 preset은 visible 4K 이상·source 60fps 이상을 최소 조건으로 사용하며, 더 큰 실제 source를 허용하고 실행 HUD에 실제 class·visible dimensions·FPS와 요구 minimum을 함께 표시해야 한다. | `VideoDecoderSelection.kt`, `LabController.kt`, `DpuLayerLabApp.kt` | `VideoDecoderSelectionTest`, `LabControllerMathTest`, `DpuLayerLabAppMathTest` |
...
| FR-SCENARIO-001 | 40개 catalog에 baseline, burst, gradual, DEVICE 후보, CLIENT 목표, video/format, resource 조합 preset을 제공해야 한다. 4K60/8K60 decoder visibility와 load surge/drop을 포함하고, catalog 명목 phase duration 중 decoder-backed 비율은 318,000/1,870,000 ms(17.01%)로 strict 10%를 초과해야 한다. | `ScenarioCatalog.kt` | `ScenarioCatalogTest` |
...
| FR-UI-002 | 실행 중 좌측 상단 HUD에 version, layer, DPU, CPU, GPU와 workload-only linear scanout-input/producer-write reference를 provenance와 함께 표시해야 한다. 현재 테스트의 실제 source class/dimensions/FPS와 요구 minimum, FIT/1:1 crop, layer·영상 metadata 회전, 이동·zoom, destination layer 크기, video/Canvas/Texture/GL producer 성격은 label이 있는 개별 줄바꿈 행으로 표시해야 한다. 최대 20개 ordered app producer를 세로 `APP PRODUCER MAP`의 V/S/T/G/F로 표시하고 planned outline과 generation-committed fill을 구분해야 한다. Decoder `ACTIVE`는 committed VIDEO producer의 generation-accepted `MediaCodec.OnFrameRenderedListener` evidence로만 표시하고 HWC assignment/DPU scanout 증거로 표현하지 않아야 한다. 상위 renderer recomposition과 분리된 immutable HUD state로 동적 text/progress 교체를 최대 1 Hz로 제한하되 topology/safety 경계는 즉시 반영해야 한다. HUD subtree는 pure Compose로 추가 Surface를 만들지 않아야 하지만 Activity root의 HWC assignment를 강제·증명한다고 표현하지 않아야 한다. | `DpuLayerLabApp.kt`, `AppProducerTopology.kt`, `FrameTracker.kt`, `LayerTrafficEstimator.kt` | UI/producer/traffic tests |
...
| FR-REPORT-001 | phase/event/sample/verdict와 provenance를 schema v2 JSON으로 원자 발행·공유해야 한다. Decoder phase는 `DECODER_PHASE_ACTIVE`와 `DECODER_FRAME_EVIDENCE` stable event type으로 source/codec과 generation-accepted callback evidence를 남기되 message를 machine schema나 HWC/DPU 증거로 사용하지 않아야 한다. | `ReportWriter.kt`, `LabController.kt`, `METRICS.md` | `ReportWriterMathTest`, controller tests |
...
| APP PRODUCER MAP planned/committed vs HWC assignment | V/S/T/G/F는 app renderer의 ordered producer 역할과 generation commit 상태이며 HWC plane 배치가 아님 |
| decoder `ACTIVE` vs DPU scanout | committed VIDEO producer의 generation-accepted `OnFrameRendered` callback freshness이며 HWC/DPU scanout 완료 증거가 아님 |
